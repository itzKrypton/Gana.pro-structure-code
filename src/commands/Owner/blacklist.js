...
const NoPrefix = require('../../models/blacklist');
...
function createEmbed(client, ID, added, allGuilds) {
      const description = added
        ...
        : `Already ${added ? 'added' : 'removed'} to my blacklist.`;
...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
      const subcommand = args[0];
        ...
        .setAuthor({ name: `blacklist add <user>\nAdd a user to my blacklist.\nblacklist remove <user>\nRemove a user from my blacklist.\nblacklist show\nShows all the users in blacklist database.\nblacklist reset\nRemoves all the users from blacklist user from the database.\``, iconURL: client.user.displayAvatarURL({ dynamic: true})})
        .setTimestamp()
      ...
      switch (subcommand) {
          case 'add': {
                ...
                if (result === 'already_added') {
                  return message.channel.send(`Already added \`${userObject.username}\` to my blacklist.`);
                } else {
                  return message.channel.send(`Added \`${userObject.username}\` to my blacklist.`);
            }
          }
          break;
          case 'remove': {
                  ...
                  const userObject = await client.users.fetch(ID);
                  if (result === 'not_found') {
                    return message.channel.send(`\`${userObject.username}\` is not in my blacklist.`);
                  } else {
                    return message.channel.send(`Removed \`${userObject.username}\` from my blacklist.`);
                  }
            }
            break;
            case 'show': {
        ...
        const embed = new EmbedBuilder()
          .setColor(client.sahilcolor)
          .setAuthor({ name: `${client.user.username}`, iconURL: client.user.displayAvatarURL({ dynamic: true})})
          .setTitle(`Total Blacklist Users - Page ${currentPage + 1}/${totalPages}`)
      ...
      case 'reset': {      
        ...
        if (!listData || listData.length === 0) {
          return message.channel.send(`No one is in my Blacklist Database.`);
        }
      
        await NoPrefix.destroy({ where: {} });

        return message.channel.send(`Reset Blacklist database.`);
      }
        default: {
